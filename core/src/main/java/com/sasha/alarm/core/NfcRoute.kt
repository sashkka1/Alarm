...
object NfcRules {
    ...
    /**
     * Уличная метка. Вешается у двери, касается на выход и на возврат.
     *
     * Тревогу не снимает и в маршруте не участвует — она нужна журналу, чтобы отличить
     * «вышел на улицу» от «уснул обратно». Двух касаний хватает, чтобы посчитать, сколько
     * времени человек пробыл снаружи; одного — только чтобы знать, что он выходил.
     */
    const val STREET_NUMBER = 0

    /**
     * Номер, который получит следующая зарегистрированная метка.
     *
     * ⚠️ **Нумерация идёт с нуля** (решение владельца 2026-08-25), и нулевая метка —
     * [STREET_NUMBER], уличная: она не снимает тревогу, а отмечает выход на улицу для
     * журнала. Поэтому первая же зарегистрированная метка получает ноль, и вешать её
     * надо у двери.
     */
    fun nextNumber(settings: NfcSettings): Int =
        settings.tags.maxOfOrNull { it.number }?.plus(1) ?: STREET_NUMBER
